...
export function registerGlobComps(app: App) {
  app.use(PButton).use(PIcon).use(PBasicHelp).use(PBasicTitle).use(PageWrapper).use(PCountTo).use(PDrawer).use(PScrollContainer).use(PSwitch).use(PSelect).use(PDropdown).use(PDescription).use(PBasicModal).use(PForm)
}
