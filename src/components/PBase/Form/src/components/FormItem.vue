...
export default defineComponent({
	...
	setup(props, { slots }) {
		...
		function handleRules() {
			...
			const getRequired = isFunction(required)
				? (required as (renderCallbackParams?: RenderCallbackParams) => void)()
				: required;
		...
		function renderComponent() {
			...
			if (isCreatePlaceholder) {
				propsData.placeholder =
					unref(getComponentProps).placeholder ||
			...
			const compSlot = isFunction(renderComponentContent)
				? {
						...(
							renderComponentContent as (
								enderCallbackParams: RenderCallbackParams,
								opts: RenderOpts
							) => any
						)(unref(getValues), {
							disabled: unref(getDisable),
		...
		function renderLabelHelpMessage() {
			...
			const getLabel = isFunction(label)
				? (label as () => string | VNode)()
				: label;
		...
		function renderItem() {
			...
			if (component === 'Divider') {
				return (
					<Col span={24}>
						<Divider {...unref(getComponentProps)}>
	...
	render() {
		...
		return (
			<Col {...this.realColProps} v-show={isShow}>
				{this.getContent()}
			</Col>
		);
	},
